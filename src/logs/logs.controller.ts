...
import type { ProcessResult } from './services/log-processor.service';
...
export class LogsController {
  ...
  @ApiResponse({ status: 200, description: 'Returns inserted/skipped/failed counters and duration' })
  ...
  @Post('export')
  @HttpCode(200)
  @ApiOperation({ summary: 'Export a CSV report by type (consumer | service | latency)' })
  @ApiBody({ type: ExportLogsDto })
  @ApiResponse({ status: 200, description: 'CSV file download', content: { 'text/csv': {} } })
  async export(@Body() dto: ExportLogsDto, @Res() res: Response): Promise<void> {
